...
function sortByType(selectors) {
  return selectors.slice().sort((a, b) => a.type.localeCompare(b.type));
}
...
describe('HTML anchoring', () => {
  ...
  beforeEach(() => {
    ...
    container.innerHTML = htmlFixture;
  ...
  testCases.forEach((testCase) => {
    it(`describes and anchors ${testCase.description}`, async () => {
      ...
      const failInfo = expectedFailures.find((f) => f[0] === testCase.description);
      let failTypes = {} as {
        range: any;
        quote: any;
        position: any;
  ...
  describe('When anchoring fails', () => {
    const validQuoteSelector: TextQuoteSelectorWithType = {
      type: 'TextQuoteSelector',
      exact: 'Lorem ipsum',
    };

    it('throws an error if anchoring using a quote fails', async () => {
      const quoteSelector: TextQuoteSelectorWithType = {
        type: 'TextQuoteSelector',
    ...
    it('does not throw an error if anchoring using a position fails', () => {
      const positionSelector: TextPositionSelectorWithType = {
    ...
    it('does not throw an error if anchoring using a range fails', () => {
      const rangeSelector: RangeSelectorWithType = {
        type: 'RangeSelector',
        startContainer: '/main',
        startOffset: 1,
        endContainer: '/main',
